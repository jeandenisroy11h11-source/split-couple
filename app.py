...
# --- GESTION DU PROFIL & COMPTEUR DE RESET ---
...
# --- 3. FORMULAIRE D'AJOUT (DISPOSITION OPTIMISÉE) ---
st.header("💰 Nouvelle Dépense")

# RANGÉE 1 : DESCRIPTION (Pleine largeur)
description = st.text_input("Où ?", placeholder="Ex: Maxi, Hydro, Essence", key=f"desc{form_suffix}")

# RANGÉE 2 : MONTANT | DATE (50/50)
col_mt, col_dt = st.columns(2)
with col_mt:
...
# RANGÉE 3 : PAYÉ PAR | TON PROFIL (50/50)
col_pay, col_prof = st.columns(2)
with col_pay:
    payer = st.selectbox("Payé par", UTILISATEURS, index=UTILISATEURS.index(st.session_state["current_user"]), key=f"payer{form_suffix}")
with col_prof:
    # On met le changement de session ici pour que ce soit clair qui utilise l'app
    nouveau_user = st.selectbox("Je suis...", UTILISATEURS, index=UTILISATEURS.index(st.session_state["current_user"]), key=f"session_top{form_suffix}")
    if nouveau_user != st.session_state["current_user"]:
        st.session_state["current_user"] = nouveau_user
        st.session_state["form_id"] += 1
        st.rerun()

# RANGÉE 4 : RÉPARTITION
...
is_periodic = st.checkbox("Dépense mensuelle récurrente", key=f"periodic{form_suffix}")
...
try:
    ...
    if not df.empty:
        ...
        # Affichage du solde basé sur "Je suis..."
        ...
        with st.expander("🔎 Historique & Suppression"):
            ...
            choix = st.selectbox("Ligne à supprimer", options=disp_df.index, format_func=lambda x: f"{disp_df.loc[x, 'Description']} ({disp_df.loc[x, 'Montant_Total']}$)")
